$(document).ready(function () {
	...
	function progressBar() {
		...
		this.displayBar = function () {
			...
			$("#explanation-equation2_3").html('<p id="p"><br>TpRetorno: ' + averageWait2 + 'ms');
			$("#explanation-equation2_4").html('<p id="p"><br>TpRespuesta: ' + averageWait3 + 'ms');
	...
	function addToBar(processName, percent, start, duration, index) {
		...
		if ($("#bar2_" + index).length == 0) {
			$(".progress2").append(" <div class='progress-bar' data-toggle='tooltip' title=' ' data-placement='right' id='bar2_" + index + "' role='progressbar' >" + processName + "</div>");
		} else {
			$("#bar2_" + index).removeClass("progress-bar-context");
		}
		$("#bar2_" + index).addClass("progress-bar-idle");
		if (processName == "context switch") {
			$("#bar2_" + index).addClass("progress-bar-context");
	...
	function createRuler(itemAmount) {
		...
		var width = $(".progress2").width();

		var spacing = (width / itemAmount) + "px";
		$(".ruler2").css("padding-right", spacing).find("li").css("padding-left", spacing);
